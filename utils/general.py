...
def non_max_suppression(
    ...
    # Checks
    if not isinstance(conf_thres, float):
        for _conf_thres in conf_thres:
    ...
    na = prediction.shape[1]
    nc = prediction.shape[2] - nm - 5  # number of classes
    if not isinstance(conf_thres, float):
        cls_pred = torch.argmax(prediction[..., 5:], dim=-1)
        _thres = conf_thres[cls_pred]
        xc = prediction[..., 4] - _thres > 0
    else:
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            if not isinstance(conf_thres, float):
                _j = j.view(-1)
                _conf = conf.view(-1)
                _thres = conf_thres[_j]
                index = _conf - _thres > 0
...
def ema_update(model1, model0, alpha=0.99, disp=False):
    for name1, p1 in model1.named_parameters():
        layer1 = name1.split('.')[1]
        for name0, p0 in model0.named_parameters():
            if name1 != name0:
                if layer1 != '24':
                    continue
                else:
                    module_name1 = name1.split('.')[2:]
                    layer0 = name0.split('.')[1]
                    module_name0 = name0.split('.')[2:]
                    if layer0 != '30' or module_name1 != module_name0:
                        continue
            p1.data = p1.data * alpha + (1 - alpha) * p0.data
            if disp:
                print('Transferred {} to {}'.format(name0, name1))
            break


def generate_mask_from_pred(pred):
    mask = pred[:, :, :, :, 4]
...
def generate_labels(pred):
    labels = []
    for index, image_pred in enumerate(pred):
        if image_pred.shape[0] > 0:
            for instance_pred in image_pred:
                temp = torch.zeros_like(instance_pred)
                temp[0] = index
                temp[1] = instance_pred[5]
                temp[2:] = xyxy2xywh(instance_pred[0:4]) / 640
                temp = temp.unsqueeze(0)
                labels.append(temp)
    if len(labels) != 0:
        for i in range(len(labels) - 1):
            labels[0] = torch.cat((labels[0], labels[i + 1]), dim=0)
        return labels[0]
    else:
        return None
...
def generate_mask_from_labels(labels, batch_size, device):
    ...
    for i in range(len(mask)):
        mask[i] = filter[i](mask[i])
    return mask


def cal_thres(conf_delta, conf_delta_square, count):
    count[count == 0] = 1

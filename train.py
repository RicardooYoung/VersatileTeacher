...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    data_dict = None
    if RANK in {-1, 0}:
        loggers = Loggers(save_dir, weights[0], opt, hyp, LOGGER)  # loggers instance
    ...
    for i in range(len(cfg)):
        check_suffix(weights[i], ".pt")  # check weights
        pretrained = weights[i].endswith(".pt")
        if pretrained:
            with torch_distributed_zero_first(LOCAL_RANK):
                weights[i] = attempt_download(
                    weights[i]
                )  # download if not found locally
            ckpt = torch.load(
                weights[i], map_location="cpu"
            )  # load checkpoint to CPU to avoid CUDA memory leak
            model.append(
                Model(
                    cfg[i] or ckpt["model"].yaml,
                    ch=3,
                    nc=nc,
            ...
            exclude = (
                ["anchor"] if (cfg[i] or hyp.get("anchors")) and not resume else []
            )  # exclude keys
            csd = ckpt["model"].float().state_dict()  # checkpoint state_dict as FP32
            csd = intersect_dicts(
                csd, model[i].state_dict(), exclude=exclude
            )  # intersect
            model[i].load_state_dict(csd, strict=False)  # load
            LOGGER.info(
                f"Transferred {len(csd)}/{len(model[i].state_dict())} items from {weights[i]}\n"
            )  # report
        else:
            model.append(
                Model(cfg[i], ch=3, nc=nc, anchors=hyp.get("anchors")).to(device)
            )  # create
        amp = check_amp(model[i])  # check AMP
        ...
        if i == 0:
            ...
            for k, v in model[i].named_parameters():
    ...
    if teacher:
        for _, v in model[1].named_parameters():
            v.requires_grad = False
        ema_update(model[1], model[0], alpha=0.0)

    # Image size
    gs = max(int(model[0].stride.max()), 32)  # grid size (max stride)
    # gs = max(8, 32)  # grid size (max stride)
    imgsz = check_img_size(opt.imgsz, gs, floor=gs * 2)  # verify imgsz is gs-multiple

    # Batch size
    if RANK == -1 and batch_size == -1:  # single-GPU only, estimate best batch size
        batch_size = check_train_batch_size(model[0], imgsz, amp)
    ...
    optimizer = smart_optimizer(
        model[0], opt.optimizer, hyp["lr0"], hyp["momentum"], hyp["weight_decay"]
    )
    ...
    # EMA
    ema = ModelEMA(model[0]) if RANK in {-1, 0} else None
    ...
    # Model attributes
    nl = (
        de_parallel(model[0]).model[-1].nl
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if RANK != -1:
            for i in range(len(train_loader)):
                train_loader[i].sampler.set_epoch(epoch)
        if joint:
            if len(train_loader[0]) < len(train_loader[1]):
                pbar = enumerate(zip(cycle(train_loader[0]), train_loader[1]))
            else:
                pbar = enumerate(zip(train_loader[0], cycle(train_loader[1])))
        else:
        ...
        for i, data in pbar:  # batch -----------------------------------------------------
            ...
            if ni - last_opt_step >= accumulate:
                ...
                torch.nn.utils.clip_grad_norm_(
                    model[0].parameters(), max_norm=10.0
                )  # clip gradients
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                for domain in range(len(val_loader)):
                    ...
                    if domain == 1:
                        ...
                        callbacks.run(
                            "on_fit_epoch_end", log_vals, epoch, best_fitness0, fi
                        )
        ...
        if (not nosave) or (final_epoch and not evolve):  # if save
            ckpt = {
                ...
                # 'model': deepcopy(de_parallel(model[0])).half(),
                "model": deepcopy(
                    de_parallel(model[0] if len(model) == 1 else model[1])
                ).half(),
...
def parse_opt(known=False):
    ...
    parser.add_argument("--weights", help="initial weights path", action="append")
    parser.add_argument("--cfg", help="model.yaml path", action="append")
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:
        ...
        opt.hyp, opt.project = check_yaml(opt.hyp), str(opt.project)  # checks
        opt.cfg = [check_yaml(opt.cfg[i]) for i in range(len(opt.cfg))]
        ...
        if opt.name == "cfg":
            opt.name = Path(opt.cfg[0]).stem  # use model.yaml as name
        opt.save_dir = str(
